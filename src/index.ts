...
// Sentinel string embedded in every dashboard frame — used to skip re-entrancy
const MARK = 'api-spy';
...
function hookStreams() {
  ...
  const ic = (chunk: any, enc: any, cb: any, orig: Function): any => {
    // Pass dashboard's own writes straight through — no erase/repaint cycle
    const s = typeof chunk === 'string' ? chunk : chunk?.toString?.() ?? '';
    if (painting || s.includes(MARK) || s.includes('┌') || s.includes('└')) return orig(chunk, enc, cb);
    erase();
...
export function apiSpy(o: { enabled?: boolean } = {}) {
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    res.on('finish', () => {
      const rp = req.route ? req.route.path : (req.baseUrl || '') + req.path;
      rEnd(req.method, rp || req.url, Number(process.hrtime.bigint() - t0) / 1e6, res.statusCode);
      erase(); paint();
    });
    next();
  };
